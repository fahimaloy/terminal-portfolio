...
export default async function handler(
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { name, provider_type, identifier_slug, api_key, base_url } =
        req.body || {};
      ...
      if (!name || !provider_type || !identifier_slug || !api_key) {
        res.status(400).json({
          ok: false,
          message:
            'name, provider_type, identifier_slug, and api_key are required',
        });
        return;
      }
      ...
      if (error) {
        if (error.code === '23505') {
          res.status(409).json({
            ok: false,
            message: 'A provider with this identifier slug already exists',
          });
          return;
        }
        throw error;
      ...
      res.status(200).json({
        ok: true,
        provider: { ...data, api_key: maskApiKey(data.api_key) },
      });
      return;
    }
    ...
    if (req.method === 'PATCH') {
      ...
      if (identifier_slug !== undefined)
        updateData.identifier_slug = identifier_slug;
