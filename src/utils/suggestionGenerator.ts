...
export class SuggestionGenerator {
  ...
  private generateBaseSuggestions(): SuggestionSource[] {
    ...
    sources.push(
      ...
      {
        id: 'system-github',
        text: 'Where can I find your GitHub?',
        category: 'system',
        priority: 8,
      },
      {
    ...
    this.projects.slice(0, 5).forEach((project, index) => {
      // Top 5 projects
      sources.push({
    ...
    this.skills.slice(0, 10).forEach((skill, index) => {
      // Top 10 skills
      sources.push({
        id: `skill-${skill.id}`,
    ...
    this.knowledgeBases.slice(0, 5).forEach((kb, index) => {
  ...
  private calculateRelevanceScore(
    ...
    const keywordMatches = source.keywords
  ...
  public generateSuggestions(
    ...
    const availableSources = sources.filter(
      (source) => !this.usedSuggestions.has(source.id),
    );
  ...
  public getSuggestionStats(): {
    ...
    return {
      ...
      availableCount: allSources.filter(
        (source) => !this.usedSuggestions.has(source.id),
      ).length,
    };
  }
