...
export function createSafeAnimatable<P extends AnimatableParams>(
  ...
  const hasAnimatedProp = Object.keys(params).some(
    (k) => !DEFAULT_KEYS.has(k) && !k.startsWith('on'),
  );
  if (!hasAnimatedProp && process.env.NODE_ENV !== 'production')
    console.warn(
      '[animatable] createAnimatable called without animated props (x/y etc) — x() will be undefined',
      params,
    );
