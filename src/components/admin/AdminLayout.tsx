...
const LogoutConfirmation: React.FC<{
  ...
  useEffect(() => {
    ...
    if (panelRef.current) {
      animate(panelRef.current, { opacity: [0, 1], scale: [0.95, 1], duration: 300, ease: 'outExpo' });
    }
  }, [isOpen]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="logout-title">
      <div ref={backdropRef} className="absolute inset-0 bg-black/80 backdrop-blur-sm opacity-0" onClick={!isLoggingOut ? onClose : undefined} />
      <div ref={panelRef} className="relative max-w-sm w-full opacity-0">
        <HudPanel accent="red" notch="md" title="// CONFIRM_LOGOUT" className="p-6">
          <div className="text-center space-y-4">
            <div className="text-5xl">🚪</div>
            <h2 id="logout-title" className="font-display tracking-[2px] text-xl text-neon-magenta text-shadow-neon-magenta">
              CONFIRM LOGOUT
            </h2>
            <p className="text-text-muted text-sm font-body">
              Are you sure you want to log out, <span className="text-neon-yellow font-display">{username}</span>?
            </p>
            <div className="flex gap-3 justify-center pt-2">
              <NeonButton variant="ghost" accent="cyan" onClick={onClose} disabled={isLoggingOut}>CANCEL</NeonButton>
              <NeonButton accent="red" onClick={onConfirm} disabled={isLoggingOut} loading={isLoggingOut}>
...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  const isActive = (path: string) => {
    if (path === '/sudosuperuser-ostaad') return router.pathname === path;
  ...
  return (
    <div className="min-h-screen bg-bg-void text-text-primary font-body">
      ...
      <header className="sticky top-0 z-40 bg-bg-void/95 backdrop-blur-md" style={{ borderBottom: '1px solid rgba(0,240,255,0.2)' }}>
        <div aria-hidden="true" className="absolute left-0 top-0 bottom-0 w-[2px] bg-neon-cyan shadow-[0_0_12px_var(--glow-cyan)]" />
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
              <div className="hidden md:flex items-center gap-2">
                ...
                <span className="inline-flex items-center px-2 py-0.5 text-[10px] font-display tracking-[2px] uppercase bg-neon-yellow/10 border border-neon-yellow/40 text-neon-yellow" style={{ clipPath: 'polygon(4px 0,100% 0,100% calc(100% - 4px),calc(100% - 4px) 100%,0 100%,0 4px)' }}>
                  ADMIN
                </span>
              </div>

              <NeonButton variant="ghost" accent="magenta" onClick={handleLogoutClick} disabled={isLoggingOut} loading={isLoggingOut} aria-label="Logout" title="Logout (Ctrl+Shift+L)">
                {isLoggingOut ? 'LOGGING OUT…' : 'LOGOUT'}
              </NeonButton>

              <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2 bg-bg-smoke text-neon-cyan transition-all" style={{ clipPath: 'var(--clip-notch-sm)' }} aria-label={mobileMenuOpen ? 'Close menu' : 'Open menu'} aria-expanded={mobileMenuOpen}>
                <span className="text-lg">{mobileMenuOpen ? '✕' : '☰'}</span>
              </button>
            </div>
          </div>

          <nav ref={desktopNavRef} className="hidden md:flex gap-1.5 mt-4 text-sm flex-wrap" aria-label="Main navigation">
            {navItems.map((item) => (
              <Link key={item.path} href={item.path}>
                <a onClick={handleNavClick} className={`admin-nav-item px-3 py-2 font-display tracking-[1.5px] uppercase text-[10px] transition-all duration-200 flex items-center gap-2 ${isActive(item.path) ? 'bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/40 hud-glow-cyan' : 'bg-transparent text-text-secondary border border-white/10 hover:border-white/30 hover:text-text-primary'}`} style={{ clipPath: 'polygon(6px 0,100% 0,100% calc(100% - 6px),calc(100% - 6px) 100%,0 100%,0 6px)' }}>
                  <span>{item.icon}</span>
                  <span>{item.label}</span>
                  {item.path === '/sudosuperuser-ostaad/meetings' && unreadMeetings > 0 && (
        ...
        {mobileMenuOpen && (
          <div ref={mobileNavRef} className="md:hidden border-t border-white/5 bg-bg-void opacity-0" aria-hidden={!mobileMenuOpen}>
            <nav className="flex flex-col p-4 gap-2 text-sm" aria-label="Mobile navigation">
              {navItems.map((item) => (
                <Link key={item.path} href={item.path}>
                  <a onClick={handleNavClick} className={`px-4 py-3 font-display tracking-[1.5px] uppercase text-[10px] transition-all flex items-center justify-between ${isActive(item.path) ? 'bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/40' : 'text-text-secondary border border-white/10 hover:border-white/30'}`} style={{ clipPath: 'polygon(6px 0,100% 0,100% calc(100% - 6px),calc(100% - 6px) 100%,0 100%,0 6px)' }}>
                    <span className="flex items-center gap-3">
                      <span className="text-lg">{item.icon}</span>
                      <span>{item.label}</span>
                    </span>
                    {item.path === '/sudosuperuser-ostaad/meetings' && unreadMeetings > 0 && (
                      <span className="bg-neon-magenta text-black text-xs font-display px-2 py-0.5">{unreadMeetings}</span>
                    )}
                  </a>
                </Link>
              ))}
        ...
        {isLoading ? (
          <div className="flex items-center justify-center min-h-[60vh]">
            <HudPanel accent="cyan" notch="md" className="p-8 text-center space-y-3">
              ...
              <div className="font-display tracking-[2px] text-neon-cyan">LOADING…</div>
            </HudPanel>
          </div>
        ) : (
          children
      ...
      <footer className="mt-auto" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-2 text-[10px] font-display tracking-[2px] uppercase text-text-muted">
            ...
            <div><code className="text-neon-magenta">$ /sudosuperuser-ostaad</code></div>
